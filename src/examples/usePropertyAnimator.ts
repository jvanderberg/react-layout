...
export const usePropertyAnimator = (
	...
	const handleAnimationStep = (startTime: number) => () => {
		...
		if (currentDuration < duration) {
			const val = (tovalue - fromvalue) * easing(currentDuration / duration) + fromvalue;
			setProperty(val);
	...
	return [property, toggle] as [number, () => void];
};
 ...
 * @param dependencies - variable references, that when changed, trigger the animation
 */
export const useOpenClose = (fromvalue: number, tovalue: number, duration: number) => {
	const [opened, setOpened] = useState<boolean>(true);
	const [valueOpened, toggleOpened] = usePropertyAnimator(fromvalue, tovalue, duration, easeInQuad);
	const [valueClosed, toggleClosed] = usePropertyAnimator(tovalue, fromvalue, duration, easeInQuad);
	...
	const res = opened ? valueOpened : valueClosed;
	return [res, toggle] as [number, () => void];
};
